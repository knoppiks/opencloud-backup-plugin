...
if [ "${1:-}" = "--purge" ]; then
  ...
  docker run --rm -v "$(pwd):/work" alpine:3 \
    sh -c 'rm -rf /work/config /work/data /work/apps /work/fixture.env /work/ca.crt' || \
    rm -rf config data apps fixture.env ca.crt
  echo "purged config/, data/, apps/, fixture.env, ca.crt"
fi
